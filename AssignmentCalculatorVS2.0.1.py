...
def calculate_grade():
    ...
    if assignment_percent > 90 and assignment_percent <= 100:
        messagebox.showinfo("Result", f"The grade you received was a {assignment_percent}% so far. You should be happy with an A; however, if not visit {html_link}.")
    elif assignment_percent > 80 and assignment_percent <= 90:
        messagebox.showinfo("Results",f"The grade you received so far in the course was a {assignment_percent}%. You should be happy with a B; however, if not review this website {html_link}.")
    elif assignment_percent > 70 and assignment_percent <= 80:
        messagebox.showinfo("Results",f'The grade received so far in the class was a {assignment_percent}%. You should raise your C by visiting {html_link}.')
    elif assignment_percent > 60 and assignment_percent <= 70:
        messagebox.showinfo("Results",f'Your current grade is {assignment_percent}%, which is a D. You can raise your grade by visiting {html_link}.')
    else:
        messagebox.showinfo("Results",f'You got an F with a low grade of {assignment_percent}%. You should immediatly visit {html_link} and start to study.')
root = tk.Tk()
