...
html_link: str = "https://varsitytutoring.com/"
...
def calculate_grade() -> None:
    ...
    for i in range(0, num_assignments, 1):
        total_window: tk.Toplevel = tk.Toplevel(root)
        total_window.geometry("450x50")
        total_text: tk.Entry = tk.Entry(total_window, width=50)
        total_text.pack()
        total_text.insert(0, f"Points possible for the {ordinal(i + 1)} assignment")

        def submit_total(event: Optional[tk.Event] = None) -> None:
            total: int = int(total_text.get())
            if total < 0:
                raise ValueError("Total must be non-negative.")
            assignment_totals.append(total)
            total_window.destroy()

        def clear_text(event: tk.Event) -> None:
        ...
        grade_window: tk.Toplevel = tk.Toplevel(root)
        grade_window.geometry("450x50")
        grade_text: tk.Entry = tk.Entry(grade_window, width=50)
        grade_text.pack()
        grade_text.insert(0, f"Grade received on the {ordinal(1 + i)} assignment")

        def submit_grade(event: Optional[tk.Event] = None) -> None:
            grade: int = int(grade_text.get())
            if grade < 0:
        ...
        def clear_grade_text(event: tk.Event) -> None:
    ...
    assignment_totals_received: int = sum(assignment_grades)
    assignment_totals_possible: int = sum(assignment_totals)
    if assignment_totals_possible != 0:
        assignment_percent: float = (
            assignment_totals_received / assignment_totals_possible
        ) * 100
    else:
        assignment_percent: int = 0
